...
int main(int argc, char *argv[])
{
	...
	pthread_t pt_producer[producerThreads];
	pthread_t pt_consumer[consumerThreads];

	int producer_id[producerThreads];
	int consumer_id[consumerThreads];

	for (int i = 0; i < producerThreads; i++) {
		producer_id[i] = i;
		pthread_create(&pt_producer[i], NULL, producer, (void *)&producer_id[i]);
	}

	for (int i = 0; i < consumerThreads; i++) {
		consumer_id[i] = i;
		pthread_create(&pt_consumer[i], NULL, consumer, (void *)&consumer_id[i]);
	}	
